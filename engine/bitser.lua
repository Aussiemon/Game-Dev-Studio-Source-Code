...
local buf_pos = 0
local buf_size = -1
local buf, writable_buf, writable_buf_size

local function Buffer_prereserve(min_size)
	if min_size > buf_size then
		buf_size = min_size
...
local function write_table(value, seen)
	local classkey
	local classname = class_name_registry[value.class] or class_name_registry[value.__baseclass] or class_name_registry[getmetatable(value)] or class_name_registry[value.__class__] or class_name_registry[value.__class]
	...
	for k in pairs(value) do
		if (type(k) ~= "number" or floor(k) ~= k or len < k or k < 1) and k ~= classkey then
			klen = klen + 1
		end
	end
...
function serialize_value(value, seen)
	if seen[value] then
...
return {
	dumps = function(value)
		...
		local result = ffi.string(buf, buf_pos)
